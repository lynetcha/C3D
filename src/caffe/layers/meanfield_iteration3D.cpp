...
void MeanfieldIteration3D<Dtype>::OneTimeSetUp(
  ...
  length_ = unary_terms->length();
  height_ = unary_terms->height();
  width_ = unary_terms->width();
  num_voxels_ = length_*height_ * width_;
  ...
  pairwise_.Reshape(num_, channels_,length_, height_, width_);
  spatial_out_blob_.Reshape(num_, channels_, length_, height_, width_);
  bilateral_out_blob_.Reshape(num_, channels_,length_, height_, width_);
  message_passing_.Reshape(num_, channels_, length_, height_, width_);
 ...
 */
template <typename Dtype>
void MeanfieldIteration3D<Dtype>::PrePass(
 ...
 */
template <typename Dtype>
void MeanfieldIteration3D<Dtype>::Forward_cpu() {
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    for (int channel_id = 0; channel_id < channels_; ++channel_id) {
      caffe_mul(num_voxels_, spatial_norm_->cpu_data(),
          spatial_out_data + channel_id * num_voxels_,
          spatial_out_data + channel_id * num_voxels_);
    }
    ...
    // Pixel-wise normalization.
    for (int channel_id = 0; channel_id < channels_; ++channel_id) {
      caffe_mul(num_voxels_, bilateral_norms_->cpu_data() + bilateral_norms_->offset(n),
          bilateral_out_data + channel_id * num_voxels_,
          bilateral_out_data + channel_id * num_voxels_);
    }
  }
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels_, num_voxels_, channels_, (Dtype) 1.,
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels_, num_voxels_,
  ...
  // sum_bottom_vec_[1]->scale_data(1);
  // const Dtype * const data = sum_bottom_vec_[0]->cpu_data();
  // std::cout << " UNARYSUM Smallest =" << *std::min_element(data,data+num_voxels_) << " largest = " << *std::max_element(data,data+num_voxels_) << std::endl;
  // const Dtype * const data1 = sum_bottom_vec_[1]->cpu_data();
  // std::cout << "PAIRWISE SUM Smallest =" << *std::min_element(data1,data1+num_voxels_) << " largest = " << *std::max_element(data1,data1+num_voxels_) << std::endl;
}


template<typename Dtype>
void MeanfieldIteration3D<Dtype>::Backward_cpu() {


  //---------------------------- Add unary gradient --------------------------
  //TODO: Check that something is not missing
  vector<bool> eltwise_propagate_down(2, true);
  sum_layer_->Backward(sum_top_vec_, eltwise_propagate_down[0], &sum_bottom_vec_);
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, channels_, channels_,
                          num_voxels_, (Dtype) 1., pairwise_.cpu_diff() + pairwise_.offset(n),
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, channels_, num_voxels_,
                          channels_, (Dtype) 1., this->blobs_[2]->cpu_data(),
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, channels_, channels_,
                          num_voxels_, (Dtype) 1., message_passing_.cpu_diff() + message_passing_.offset(n),
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, channels_, num_voxels_, channels_, (Dtype) 1.,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    for (int channel_id = 0; channel_id < channels_; ++channel_id) {
      caffe_mul(num_voxels_, spatial_norm_->cpu_data(),
                spatial_out_diff + channel_id * num_voxels_,
                spatial_out_diff + channel_id * num_voxels_);
    }
    ...
    for (int channel_id = 0; channel_id < channels_; ++channel_id) {
      caffe_mul(num_voxels_, bilateral_norms_->cpu_data() + bilateral_norms_->offset(n),
                bilateral_out_diff + channel_id * num_voxels_,
                bilateral_out_diff + channel_id * num_voxels_);
    }
  }
  ...
  //--------------------------------------------------------------------------------
 // TODO: Check that something is not omitted
  vector<bool> propagate_down(2, true);
  softmax_layer_->Backward(softmax_top_vec_, propagate_down[0], &softmax_bottom_vec_);
}

INSTANTIATE_CLASS(MeanfieldIteration3D);
}  // namespace caffe

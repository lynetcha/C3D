...
void EltwiseLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 1; i < bottom.size(); ++i) {
    CHECK(bottom[i]->num() == bottom[0]->num());
    CHECK(bottom[i]->channels() == bottom[0]->channels());
    CHECK(bottom[i]->length() == bottom[0]->length());
    CHECK(bottom[i]->height() == bottom[0]->height());
    CHECK(bottom[i]->width() == bottom[0]->width());
  }
  top[0]->ReshapeLike(*bottom[0]);
  // If max operation, we will initialize the vector index part.
  if (this->layer_param_.eltwise_param().operation() ==
      EltwiseParameter_EltwiseOp_MAX && top.size() == 1) {
    max_idx_.Reshape(bottom[0]->num(),bottom[0]->channels(),bottom[0]->length(),bottom[0]->height(),bottom[0]->width());
  }
}

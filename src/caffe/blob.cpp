...
void Blob<Dtype>::ReshapeLike(const Blob<Dtype>& other) {
  Reshape(other.num(), other.channels(), other.length(), other.height(), other.width());
}

template <typename Dtype>
Blob<Dtype>::Blob(const int num, const int channels, const int length, const int height,
    const int width) {
  Reshape(num, channels, length, height, width);
}
...
void Blob<Dtype>::CopyFrom(const Blob& source, bool copy_diff, bool reshape) {
  if (num_ != source.num() || channels_ != source.channels() || length_ != source.length() ||
      height_ != source.height() || width_ != source.width()) {
    if (reshape) {
      Reshape(source.num(), source.channels(), source.length(), source.height(), source.width());
    } else {
...
void Blob<Dtype>::FromProto(const BlobProto& proto) {
  if (proto.length())
	Reshape(proto.num(), proto.channels(), proto.length(), proto.height(), proto.width());
  else // for backward compatibility
	  Reshape(proto.num(), proto.channels(), 1, proto.height(), proto.width());
  // copy data
